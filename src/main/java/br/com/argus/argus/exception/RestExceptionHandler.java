...
import br.com.argus.argus.responses.ErrorResponse;
import br.com.argus.argus.responses.ObjectError;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private List<ObjectError> getErrors(MethodArgumentNotValidException ex) {
		return ((Collection<ObjectError>) ex.getBindingResult().getFieldError()).stream()
				.map(error -> new ObjectError(error.getMessage(), error.getField(), error.getParameter()))
